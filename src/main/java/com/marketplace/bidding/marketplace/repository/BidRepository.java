...
public interface BidRepository extends CrudRepository<Bid,Long>{
  ...
  @Query(value = "SELECT * FROM BID WHERE buyer_user_id=:buyerUserId and AMOUNT = (select min(amount) from bid, project where project.id=bid.project_id and project.bidding_end_time <= now())", nativeQuery = true)
  List<Bid> getWinningBidForBuyerId(@Param("buyerUserId") Long buyerUserId);
